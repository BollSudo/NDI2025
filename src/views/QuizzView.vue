...
const getProfileDescription = computed(() => {
  const score = autonomyScore.value
  if (score < 25) return 'Vous commencez votre parcours. Explorez les solutions faciles pour démarrer !'
  if (score < 50) return 'Bon début ! Continuez avec l\'expérimentation de nouvelles solutions.'
  if (score < 75) return 'Excellent progrès ! Vous êtes sur la bonne voie vers l\'autonomie complète.'
...
<template>
  <div class="w-screen min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white flex flex-col gap-2 items-center justify-center py-6">
  <section id="form" class="w-full flex items-center justify-center">
    <div class="container max-w-4xl">
      ...
      <div class="bg-white/5 backdrop-blur-lg rounded-2xl p-8 border border-white/10">
        ...
        <div class="mt-8 p-6 bg-gradient-to-r from-blue-900/50 to-green-900/50 rounded-xl border border-blue-500/30 flex items-center">
          <div class="flex-1">
            <h4 class="font-bold text-lg mb-2">🎯 Votre Profil: {{ getProfileLevel }}</h4>
            <p class="text-sm text-gray-300">{{ getProfileDescription }}</p>
          </div>
          <div class="rounded-full w-20 h-20 border-2 top-3 right-0 flex items-center justify-center">
        ...
        <div class="mb-8">
          ...
          <div class="w-full bg-gray-700 rounded-full h-4">
            <div class="bg-gradient-to-r from-blue-500 via-green-500 to-purple-500 h-4 rounded-full transition-all duration-500"
