...
interface Pillar {
  icon: string,
  model: string,
  camera: [number, number, number] | Array<number>,
...
<template>
  <div class="flex flex-col items-center gap-2 w-full py-4">
    <div class="text-5xl mb-4 w-50 h-50">
      <Suspense>
        <RotatingModel :model-path="pillar.model" :camera-position="pillar.camera" :ambient-light-intensity="pillar.ambientLightIntensity" ></RotatingModel>
        <template #fallback>
